...
export const getConfigParameterStore = defineStore("configParameterStore", () => {
    ...
    for (const param of parameters.value) {
      if (
        ...
        if (
          ...
          if (param.baseInformation?.name.toLowerCase() === key.toLowerCase()) {
            return param;
          }
        } else {
  ...
  function newParameter(): ConfigParameter {
    ...
    async function saveParameter(paramater: ConfigParameter) {
        if (paramater.baseInformation?.id !== undefined && paramater.baseInformation.id.uuid.length > 0) {
  ...
  return { parameters, getAllParameters, getCachedParameter, startListening, saveParameter, deleteParameter, newParameter };
});
